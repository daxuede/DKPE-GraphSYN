        ...
        if self.dataset == 'tdcDDI':
            ...
            for i,data in X.items():
                data.id = torch.tensor(
                    [i])  # id here is the index of the mol in
                # the dataset
                data.y = torch.tensor([y[i]], dtype=torch.float64)
...
def get_stratified_partitions(y, num_folds=5, valid_set_portion=0.1, random_state=42):
    ...
    Args:
        y: deepadr labels
    """
    skf_trte = StratifiedKFold(n_splits=5, random_state=random_state, shuffle=True)  # split train and test
    
    skf_trv = StratifiedShuffleSplit(n_splits=num_folds,
